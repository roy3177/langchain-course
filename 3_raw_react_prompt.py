...
def get_tool_descroptions(tools_dict):
    descriptions=[]
...
tool_descriptions=get_tool_descroptions(tools)
...
3. NEVER calcu;ate discounts yourself using math. Always use the apply_discount tool .
...
def run_agent(question:str):
    ...
    for iteration in range(1, MAX_ITERATIONS + 1): 
        ...
        final_answer_matchch=re.search(r"Final Answer:\s*(.+)", output)

        if final_answer_matchch:
            final_answer=final_answer_matchch.group(1).strip()
        ...
        if not action_match or not action_input_match:
            print(
                " [Parsing] Error: Could not find Action or Action Input in LLM output."
            )
            break
        ...
        print(f" [Tool Exeuting] {tool_name}({args})... ")
